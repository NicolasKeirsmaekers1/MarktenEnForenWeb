...
export class StandplaatsComponent implements OnInit {
    ...
    bewaren() {
        ...
        if (this.standplaatsForm.get("producten").value.some(x => x.isHoofdcategorie)) {
            if (status.code == "LOPEND" || status.code == "OPGESCHORT") {
                ...
                var localISOTime = (new Date(ctrl.value- offset)).toISOString().slice(0, -1);
                ...
                if (kavelDatum < new Date().addDays(30)) {
                    this.modal.confirm()
                        ...
                        .then(x => {
                            ...
                            this.abonnementSvc.post(this.data.id, returnValue)
                                .subscribe(x => {
                ...
                this.toastr.error("Deze actie is niet mogelijk op een abonnement met status " + status.omschrijving, "Kavels verminderen");
            }
        } else {
    ...
    getKavels(): void {
        ...
        this.kavelService.getAllKavelsForAbonnement(this.data.markt.Id, this.data.id)
                        .subscribe( kavels => {
                            this.kavels = kavels;
    ...
    mousedown($event): void {
        ...
        setTimeout(() => {
            ...
            this.kavelService.getKavelDetail(this.data.markt.id, kavelId).subscribe(x => {
                this.kavel = x;
    ...
    OpenPanelsStandplaats(): void {
        ...
        if (linkProducten && linkProducten[0] && !panelProducten.classList.contains('in')) { linkProducten[0].click() }

        var linkKavels = document.getElementsByName('kavels');
        var panelKavels = document.getElementById('kavels');

        if (linkKavels && linkKavels[0] && !panelKavels.classList.contains('in')) { linkKavels[0].click() }
    ...
    cancelScaleDown(): void {
        if (this.kavels.some(x => x.kavelStatus.toString().lastIndexOf("BEZET") !== 0 || x.kavelStatus == KavelStatusEnum.BezetGereserveerd)) {
            this.modal.alert()
                ...
                .catch() // catch error not related to the result (modal open...)
                ;
        } else if (this.kavels.every(x => x.kavelStatus == KavelStatusEnum.Bezet|| x.kavelStatus == KavelStatusEnum.BezetTeBedelen)) {
            var returnValue = this.standplaatsForm.value;
