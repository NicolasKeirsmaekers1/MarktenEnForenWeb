...
import { AbonnementService, IKavelIdentifierDto, KavelService, KavelStatusEnum } from '../services';
...
@Component({
    ...
    styles: [`
        .right-slider {
            ...
            -ms-opacity: 0;
            opacity: 0;
            -ms-transition: right .5s cubic-bezier(0, 1, 0.5, 1), opacity .3s cubic-bezier(0, 1, 0.5, 1);
            -o-transition: right .5s cubic-bezier(0, 1, 0.5, 1), opacity .3s cubic-bezier(0, 1, 0.5, 1);
            -webkit-transition: right .5s cubic-bezier(0, 1, 0.5, 1), opacity .3s cubic-bezier(0, 1, 0.5, 1);
...
export class StatusComponent implements OnInit {
    ...
    private 
    @Output() 
    @ViewChild("aardSelector", {static: false}) aardSelector: CodeSelectComponent;
    @ViewChild("redenSelector", {static: false}) redenSelector: CodeSelectComponent;
    ...
    ngOnInit(): void {
        this.statusForm = this.fb.group({
            ...
            aard: this.fb.control({ value: "", disabled: true }, [Validators.required]),
            reden: this.fb.control({ value: "", disabled: true }, [Validators.required]),
            opmerkingen: this.fb.control(""),
            beginDatum: this.fb.control({ value: new Date(), disabled: true }, [Validators.required]),
            eindDatum: this.fb.control({ value: new Date(), disabled: true }, [Validators.required]),
            ondernemingsnummerOvernemer: this.fb.control({ value: "", disabled: true }, [Validators.required]),
            eindDatumHuidigAbonnenemt: this.fb.control({ value: new Date(), disabled: true }, [Validators.required]),
            ontvangstAanvraagOpschortingDatum: this.fb.control({ value: new Date(), disabled: true }, [Validators.required]),
            eindDatumAbonnement: this.fb.control({ value: new Date(), disabled: true }, [Validators.required]),
            briefBetekeningOpzegDatum: this.fb.control({ value: new Date(), disabled: true }, [Validators.required]),
            datumOntvangstOpzeg: this.fb.control({ value: new Date(), disabled: true }, [Validators.required])
        });
    ...
    private resetFormGroup() {
        ...
        for (let ctrlName in this.statusForm.controls) {
            if (excludes.includes(ctrlName)) {
                ...
                if (ctrlName === "opmerkingen") this.statusForm.get(ctrlName).setValue("");
            } else {
    ...
    private ondernemingFound($event: IMessageEnrichedModel<any>) {
        if (!$event.data || $event.data.id <= 0) {
            this.modal.confirm()
                ...
                .then(x => {
                    this.onderneming = {
                        ondernemingsnr: this.statusForm.get("ondernemingsnummerOvernemer").value
                    };
    ...
    klantCreated($event: any) {
        this.statusForm.patchValue({ ondernemingsnummerOvernemer: $event.ondernemingsnr });
    ...
    onBeforeRenderRedenen = (x: ICodeData) => {
        ...
        switch (this.status.toUpperCase()) {
            case "GESCHORST":
                ...
                break;
            case "INOVERDRACHT":
                filter = ["STOPZETTINGZAAKOFLEURHANDEL", "OVERLIJDENZAAKVOERDER", "ECHTSCHEIDING", "ANDERE"];
                break;
            case "OPGESCHORT":
            ...
            case "INGETROKKEN":
                ...
                break;
            case "LOPEND":
                if (this.currentStatus === "OPGESCHORT" || this.currentStatus === "INOVERDRACHT" || this.currentStatus === "INGETROKKEN" || this.currentStatus === "OPGEZEGD") {
    ...
    security(): void {
        this.canUpdate =
            this.auth.hasPermission(Privilege.Abonnement.Update).pipe(map(x => {
                const states = [ApplicationConstants.AbonnementStatus.LopendCode,
    ...
    save(): void {
        ...
        if (this.status === "INOVERDRACHT") {
            var kavelsFromForm = this.statusForm.get("overTeNemenKavelIds") as FormArray;
        ...
        else if (this.status === "LOPEND" && this.currentStatus === "OPGESCHORT") {
            this.modal.confirm()
    ...
    private persist(value?: any): void {
        ...
        this.abonnementSvc.wijzigStatus(this.data.id, this.status, data, this.files).subscribe(x => {
            this.data.abonnementStatusBeginDatum = data.startDatumEersteOpschorting || data.beginDatum || data.datumOntvangstOpzeg || new Date();
            this.data.abonnementStatusEindDatum = data.eindDatum || data.eindDatumAbonnement || null
            ...
            this.toastr.error("Kon de abonnement wijzigingen niet doorvoeren", "Wijzig Status");
                if (x.ExtraInfo) {
    ...
    private kavelsBeschikbaarVoorTerugLopend(): boolean {
        for (let item of this.kavels) {
            if (item.kavelStatus === KavelStatusEnum.Gereserveerd|| KavelStatusEnum.BezetGereserveerd) {
                return false;
            }
        }
        return true;
    ...
    private getKavels(): void {
        this.kavelService.getAllKavelsForAbonnement(this.data.markt.id, this.data.id).subscribe(kavels => {
            this.kavels = kavels;

            if (this.statusForm.get("overTeNemenKavelIds"))
                this.statusForm.removeControl("overTeNemenKavelIds");

            this.statusForm.addControl("overTeNemenKavelIds", this.fb.array(this.kavels.map(x => {
                x["enabled"] = false;
                return {
                    value: x.id,
                    disabled: true
    ...
    showPersistModal() {
        var message = "De startdatum van uw " + (this.status === "GESCHORST" ? "schorsing" : "opschorting") + " ligt in het verleden. Wenst u hiermee verder te gaan?";
