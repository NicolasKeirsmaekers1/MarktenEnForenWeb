  ...
  template: `
    <div #input class="input-group date">
      <input
        ...
        [attr.readonly]="disabled == true ? true : null"
      />
  ...
  ngAfterViewInit() {
    ...
    this._jqElement = (<any>jQuery(this.input.nativeElement)).datepicker({
      ...
      beforeShowDay: function (date) {
        if (dates) {
          datepair.enabled = dates.some(
            (x) =>
              x.getDate() == date.getDate() && x.getMonth() == date.getMonth()
          );
          datepair.classes = '';
        }
    ...
    if (this.value)
      this._jqElement.datepicker(
        'setDate',
        this.formatDate(new Date(this.value))
      );
    this._jqElement
      .on('changeDate', (e) => {
        this.value = new Date(
          new Date(new Date(e.date).toString()).setHours(0)
        );
      })
      .on('clearDate', (e: any) => {
        this.value = null;
