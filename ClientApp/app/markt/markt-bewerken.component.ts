...
} from './kavel-detail.modal.component';
...
export class MarktBewerkenComponent {
  ...
  getKavels(id: number): void {
    this.kavelService.getAllKavelsForMarktOverview(id).subscribe(
      (kavels) => {
        this.kavels = kavels;
        this.kavels.sort(function (a, b) {
  ...
  uploadKavelsCsv(event) {
    ...
    if (fileList.length > 0) {
      ...
      reader.onloadend = (e: any) => {
        this.kavelService.uploadKavels(this.markt.id, this.document).subscribe(
          (x) => {
  ...
  updateMarkt() {
    ...
    this.marktService.save(this.bewerkMarktForm.value).subscribe(
      (markt) => {
        this.toastr.success(
          this.markt.id > 0
    ...
    this.modal
      .open(
        ...
        overlayConfigFactory(
          new KavelDetailModalContext(
            kavelId,
            this.markt.id,
            this.hideKlant(kavelId)
          ),
          KavelDetailModalContext,
          {
  ...
  hideKlant(kavelId: number): boolean {
    return kavelId === 0
      ? true
      : this.kavels.find((x) => x.id == kavelId).kavelStatus ===
          KavelStatusEnum.Vrij;
  }
