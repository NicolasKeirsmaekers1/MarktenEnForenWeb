...
export class ProductService extends BaseService {
    ...
    getProducten(query?: any): Observable<IProduct[]> {
        ...
        if (query) {
            if (query.sort) httpParams = httpParams.set("sort", query.sort.toString());
            if (query.page) httpParams = httpParams.set("page", query.page.toString());
            if (query.pageSize) httpParams = httpParams.set("pageSize", query.pageSize.toString());
        }
    ...
    updateWithArray(payload: IProduct[]): Observable<any> {
        this.loader.start();
        return this.http.post<Response>(this.url + "/update", payload, { headers: this.jsonHeaders }).pipe(
            tap(x => { this.loader.complete(); }),
            catchError(this.handleError));
    }
    ...
    update(payload: IProduct): Observable<any> {
        this.loader.start();
        return this.http.put<Response>(this.url + "/" + payload.id, payload, { headers: this.jsonHeaders }).pipe(
            tap(x => { this.loader.complete(); }),
            catchError(this.handleError));
    }
    ...
    save(payload: IProduct): Observable<any> {
        this.loader.start();
        return this.http.post<Response>(this.url, payload, { headers: this.jsonHalHeaders }).pipe(
            tap(x => { this.loader.complete(); }),
            catchError(this.handleError));
    }
