...
export class WachtlijstIntakeDetailComponent implements OnInit {
    ...
    @ViewChild(MarktPanelComponent, {static: false}) private marktPanel: MarktPanelComponent;
    @ViewChild(ProductPanelComponent, {static: false}) private productPanel: ProductPanelComponent;
    ...
    ngOnInit(): void {
        ...
        this.klantId = this.intake.statusCode === ApplicationConstants.AanvraagStatus.Intake && this.intake.id === 0
            ? this.klant.id
            : this.intake.klantId;
        ...
        if (typeof this.intake.deletedOn !== 'undefined' && this.intake.deletedOn != null) {
            this.deleted = true;
        ...
        if (this.intake.soortCode == ApplicationConstants.AanvraagSoort.PlaatsKwijtgeraaktCode) {
    ...
    private buildForm(): void {
        this.intakeForm = this.fb.group({
            'id': [this.intake.id],
            'soortCode': [{ value: this.intake.soortCode, disabled: this.intake.id > 0 }, [Validators.required]],
    ...
    getTotaalAantalKavels(aanvraagMarkten: any): void {
        ...
        if (this.intake.soortCode == ApplicationConstants.AanvraagSoort.PlaatsKwijtgeraaktCode || this.intake.soortCode == ApplicationConstants.AanvraagSoort.NieuwePlaatsCode) {
    ...
    marktenChanged($event: Array<any>) {
        let totaalAantalKavels = $event.map(x => x.aantalKavels).reduce((a, b) => a + b, 0);
        if (this.intake.soortCode == ApplicationConstants.AanvraagSoort.PlaatsKwijtgeraaktCode || this.intake.soortCode == ApplicationConstants.AanvraagSoort.NieuwePlaatsCode || this.intakeForm.get("soortCode").value == ApplicationConstants.AanvraagSoort.NieuwePlaatsCode || this.intakeForm.get("soortCode").value == ApplicationConstants.AanvraagSoort.PlaatsKwijtgeraaktCode) {
    ...
    statusChange() {
        ...
        if (!this.validateProducten() &&
            (this.intakeForm.value.soortCode === ApplicationConstants.AanvraagSoort.NieuwePlaatsCode || this.intakeForm.value.soortCode === ApplicationConstants.AanvraagSoort.PlaatsKwijtgeraaktCode)) {
    ...
    registreren() {
        ...
        if (this.intakeForm.value.isBetaald == null || this.intakeForm.value.isBetaald === false) {
    ...
    setModus() {
        this.directBewarenButton = this.auth.hasPermission(Privilege.Aanvraag.DirectRegistreren);
        ...
        switch (this.intake.statusCode) {
            case ApplicationConstants.AanvraagStatus.Intake:
                if (this.intake.id === 0) {
                    ...
                    this.statusChangeButton = this.auth.hasPermission(Privilege.Aanvraag.VeranderNaarGepreregistreerd);
                ...
                break;
            case ApplicationConstants.AanvraagStatus.Gepreregistreerd:
                this.title = "Pre-registratie bewerken > " + this.intake.klant.firmanaam;
                this.bewaarButtonText = "Pre-registratie bewaren";
                this.bewaarButton = this.auth.hasPermission(Privilege.Aanvraag.PreregistratieBewerken);
                this.statusChangeButton = this.auth.hasPermission(Privilege.Aanvraag.VeranderNaarGepreregistreerd);
            ...
            case ApplicationConstants.AanvraagStatus.Geregistreerd:
                ...
                this.bewaarButton = this.auth.hasPermission(Privilege.Aanvraag.RegistratieBewerken);
                this.deleteButton = this.auth.hasPermission(Privilege.Aanvraag.RegistratieVerwijderen);
                break;
        }
    }
    ...
    showPreregistratieModal() {
        this.modal.confirm()
            ...
            .then(x => this.aanvraagService
                .changeStatus(this.intake.id, ApplicationConstants.AanvraagStatus.Gepreregistreerd, this.intakeForm.getRawValue())
                .subscribe(x => {
                    this.router.navigate(["wachtlijst", "preregistratie"]);
                }, x => {
                    this.toastr.error("Kan aanvraag niet naar 'gepreregistreerd' zetten.");
        ...
        this.modal.confirm()
            ...
            .then(x =>
                this.aanvraagService
                    .changeStatus(this.intake.id, ApplicationConstants.AanvraagStatus.Geregistreerd, this.intakeForm.getRawValue())
                    .subscribe(x => {
    ...
    showRegistratieDeleteModal() {
        ...
        var geldig = this.intake.officieleInschrijvingsDatum != null
            && new Date(currentDate) <= new Date(this.intake.officieleInschrijvingsDatum)
            && new Date(currentDate.setFullYear(currentDate.getFullYear() + 1)) >= new Date(this.intake.officieleInschrijvingsDatum);

        if (geldig)
            text += "Het inschrijvingsrecht is nog steeds geldig. ";

        text += "Weet u zeker dat u deze aanvraag wilt verwijderen? Deze actie is onomkeerbaard.";
        ...
        this.modal.confirm()
            ...
            .then(dialog => dialog.result) // dialog has more properties, let's just return the promise for a result. 
            .then(x =>
                this.aanvraagService.delete(this.intake.id)
                    .subscribe(x => {
    ...
    showDirectModal() {
        if (this.intakeForm.value.aanvraagMarkten && this.intakeForm.value.aanvraagMarkten.length > 1) {
            ...
            .then(x =>
                this.aanvraagService
                    ...
                    .subscribe(x => {
                        ...
                        this.toastr.error("Kan aanvraag niet naar 'geregistreerd' zetten.");
                        var error = "de status kan niet veranderd worden.";
                        if (this.toastr.error.length != error.length) {
                            this.displayErrors(x);
                        }
    ...
    changeSoortCode($event) {
        ...
        switch ($event) {
            ...
            case ApplicationConstants.AanvraagSoort.PlaatsKwijtgeraaktCode:
                ...
                this.abonnementSearchCriteria.statusCode = ApplicationConstants.AbonnementStatus.IngetrokkenCode;
                this.abonnementService.getAll(this.page, this.abonnementSearchCriteria).subscribe(abonnementen => {
                    if (abonnementen.embedded.resourceList.length > 0) {
                        this.intakeForm.get("uitstallingCode").setValue(abonnementen.embedded.resourceList[0].uitstallingCode);
                        this.intakeForm.get("verkoopCode").setValue(abonnementen.embedded.resourceList[0].verkoopCode);
                        this.intakeForm.get("elektriciteitCode").setValue(abonnementen.embedded.resourceList[0].elektriciteitCode);
                        this.maxKavels = 0;
                        ...
                        this.toastr.info("Deze klant heeft geen ingetrokken abonnementen.");
                        this.intakeForm.get("soortCode").setValue(ApplicationConstants.AanvraagSoort.NieuwePlaatsCode);
                    }
                });
                break;
            default:
    ...
    onSubmit(): void {
        ...
        this.aanvraagService.save(this.intakeForm.getRawValue()).pipe(map((response: Response) => {
            ...
            this.toastr.error("Aanvraag kon niet worden bewaard.", "Aanvraag bewaren");
            this.displayErrors(x);
        });
    }
    ...
    validateMarkten(): boolean {
        return this.intakeForm.value.aanvraagMarkten && this.intakeForm.value.aanvraagMarkten.length > 0;
    }

    validateProducten(): boolean {
        return this.intakeForm.value.producten && this.intakeForm.value.producten.length > 0;
    }
