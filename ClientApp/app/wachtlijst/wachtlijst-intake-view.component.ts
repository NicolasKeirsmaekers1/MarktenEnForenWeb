...
import { AbonnementService, IKavelIdentifierDto, KavelService } from '../services';
import { AanvraagWeigeringService, IWeigeringsTermijn } from '../services/aanvraag-weigering.service';
...
export class WachtlijstIntakeViewComponent {
    ...
    onload(): void {
        ...
        if (this.preregistratie) {
            this.routerlink = "/wachtlijst/preregistratie/" + this.intake.id + "/edit";
    ...
    setLabels(): void {
        // set status label
        if (this.intake.statusCode === ApplicationConstants.AanvraagStatus.Gereserveerd) {
        ...
        if (this.intake.statusCode === ApplicationConstants.AanvraagStatus.Gepreregistreerd) {
            this.preregistratie = true;
            this.status = ApplicationConstants.AanvraagStatus.Gepreregistreerd;
        }

        if (this.intake.statusCode === ApplicationConstants.AanvraagStatus.Geregistreerd) {
            this.registratie = true;
            this.status = ApplicationConstants.AanvraagStatus.Geregistreerd;
        }

        if (this.intake.statusCode === ApplicationConstants.AanvraagStatus.Stopgezet) {
            this.stopgezet = true;
            this.status = ApplicationConstants.AanvraagStatus.Stopgezet;
        }

        if (this.intake.statusCode === ApplicationConstants.AanvraagStatus.Geannuleerd) {
    ...
    selectKavel($event: Event, aanvraagMarkt: any, kavel: IKavelIdentifierDto) {
        ...
        if (kavel.kavelStatus === KavelStatusEnum.Bezet || kavel.kavelStatus === KavelStatusEnum.BezetGereserveerd || kavel.kavelStatus === KavelStatusEnum.Gereserveerd) return;
    ...
    security(): void {
        ...
        this.toewijzenAanvragenNietInAanmerking = this.auth.hasPermission(Privilege.Aanvraag.ToewijzenAanvragenNietInAanmerking)
    }
    ...
    fetchWeigeringData(page: IQueryParameters): void {
        this.aanvraagWeigeringService.getAll(this.intake.id).subscribe((x: IPagedResult<IWeigeringsTermijn>) => {
            this.dataWeigering = x;
            this.kanWeigeringStarten = x.embedded.resourceList.findIndex(x => !x.weigeringsDatum) === -1;
        }, x => this.toastr.error("Kon de Weigeringstermijnen niet inladen", "Termijn"));
    }
    ...
    startWeigeringen(): void {
        this.weigeringForm = this.fb.group({
            ...
            eindDatum: [{ value: new Date(Date.now()).addMonths(1).addDays(-1), disabled: true }]
        });
    ...
    submitWeigering($event) {
        ...
        if (this.startWeigeringendisplay) {
            this.modal.confirm()
                ...
                .then(dialog => dialog.result) // dialog has more properties,lets just return the promise for a result. 
                .then(x => {
                    this.aanvraagWeigeringService.starten(this.intake.id, this.weigeringForm.value).pipe(
                        tap(b => {
                            this.weigeringForm = null;
                            this.startWeigeringendisplay = false;
                        }))
                        .subscribe(b => {
                            ...
                            this.toastr.error("Kon de Weigeringstermijn niet starten", "Termijn");
                            if (b.ExtraInfo) {
    ...
    weigeringAnnuleren(data: any): void {
        this.aanvraagWeigeringService.registreren(this.intake.id, data).pipe(
            tap(x => {
                this.fetchWeigeringData(null);
            }))
            .subscribe(x => {
                this.toastr.success("Weigeringstermijn is geannuleerd", "Weigeringstermijn");
            }, x => {
                this.toastr.error("Kon de weigeringstermijn niet annuleren", "Weigeringstermijn");
                if (x.ExtraInfo) {
                    for (let xtra in x.ExtraInfo) {
    ...
    weigeringRegistreren(data: any): void {
        this.aanvraagWeigeringService.registreren(this.intake.id, data).pipe(
            ...
            .subscribe((x: IWeigeringsTermijn) => {
